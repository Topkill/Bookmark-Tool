...
import re                       # 导入正则表达式库
import html                     # 导入html库用于解码HTML实体

def parse_bookmarks_file(filepath, mode, filter_enabled):
    """
    【健壮正则版】使用正则表达式解析文件。
    - 增加了 html.unescape 来处理 & 等HTML实体，保证结果准确。
    """
    print(f"正在以 '{mode}' 模式健壮正则解析文件: {filepath} (过滤: {'启用' if filter_enabled else '禁用'}) ...")
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
    except FileNotFoundError:
        print(f"错误: 文件 '{filepath}' 未找到。")
        return None, 0, None, 0 # 返回一致的错误格式
    ...
    for raw_url, raw_title in raw_bookmarks:
        if raw_url:  # 增加一个判断，以防空的href属性
            # 【核心修正】
            # 1. 解码HTML实体 (e.g., '...&...' -> '...&...')
            decoded_html_url = html.unescape(raw_url)
            # 2. 解码URL百分号编码 (e.g., '%20' -> ' ')
            normalized_url = unquote(decoded_html_url)
            ...
            if should_add:
                if mode == 'url':
                    ...
                    # 【修正】对标题也进行同样的处理
                    decoded_html_title = html.unescape(raw_title)
                    clean_title = re.sub(r'<.*?>', '', decoded_html_title, flags=re.DOTALL).strip()
...
def generate_report(file1, file2, output_file, mode, filter_enabled):
    """
    【无需改动】报告生成函数。
    """
    start_time = time.time()
    
    bookmarks1_set, total_bookmarks1, folders1_set, total_folders1 = parse_bookmarks_file(file1, mode, filter_enabled)
    if bookmarks1_set is None: return

    bookmarks2_set, total_bookmarks2, folders2_set, total_folders2 = parse_bookmarks_file(file2, mode, filter_enabled)
...
def main():
    ...
    generate_report(args.file1, args.file2, output_file, args.mode, args.filter)
