...
def parse_bookmarks_file(filepath, mode, filter_enabled):
    ...
    - 新增 filter_enabled 开关来控制是否过滤链接。
    - 新增 URL 解码功能来修复编码不一致的bug。
    """
    print(f"正在以 '{mode}' 模式解析文件: {filepath} (过滤: {'启用' if filter_enabled else '禁用'}) ...")
    try:
    ...
    for a_tag in all_a_tags:
        ...
        # 只要URL存在就进行处理
        if url:
            # 【BUG修复】统一对URL进行解码，解决编码不一致问题
            normalized_url = unquote(url)

            # 【新增】根据开关决定是否进行过滤
            should_add = True # 默认不过滤，直接添加
            if filter_enabled:
                # 如果开启过滤，则检查协议
...
def generate_report(file1, file2, output_file, mode, filter_enabled):
    ...
    report_lines.append(f"  - 总书签数: {total_bookmarks1} (唯一书签数: {len(bookmarks1_set)})")
    report_lines.append(f"  - 总文件夹数: {total_folders1} (唯一文件夹数: {len(folders1_set)})")
...
def main():
    ...
    parser.add_argument(
        "-f", "--filter",
        action='store_true', # 当出现--filter时，该参数值为True
