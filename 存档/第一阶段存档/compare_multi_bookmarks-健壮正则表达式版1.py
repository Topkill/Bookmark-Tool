...
【最终统一版】多书签文件对比脚本
- 使用正则表达式，无需第三方库，性能高。
- 增加了HTML实体解码，确保结果与专业解析器(BeautifulSoup)一致。
- URL解码功能变为可选。
"""
import argparse
import time
...
def parse_bookmarks_file(filepath, mode, filter_enabled, decode_enabled):
    ...
    """
    print(f"正在以 '{mode}' 模式健壮正则解析文件: {filepath} (过滤: {'启用' if filter_enabled else '禁用'}, 解码: {'启用' if decode_enabled else '禁用'}) ...")
    try:
    ...
    processed_folders = raw_folders
    if decode_enabled:
        processed_folders = [html.unescape(f) for f in raw_folders]
    folders_set = {re.sub(r'<.*?>', '', f, flags=re.DOTALL).strip() for f in processed_folders}
    ...
    for raw_url, raw_title in raw_bookmarks:
        if raw_url:
            ...
            # 【核心改动】根据开关决定是否解码
            if decode_enabled:
                normalized_url = unquote(html.unescape(raw_url))
                processed_title = html.unescape(raw_title)
            ...
            if should_add:
                if mode == 'url':
                    ...
                    clean_title = re.sub(r'<.*?>', '', processed_title, flags=re.DOTALL).strip()
