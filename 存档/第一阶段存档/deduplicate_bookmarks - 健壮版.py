...
def parse_bookmarks(filepath, decode_enabled):
    ...
    for raw_url, raw_title in raw_bookmarks:
        if raw_url:
            ...
            # 清理标题中可能存在的HTML标签（虽然正则已经尽力避免，但以防万一）
...
def deduplicate_and_generate_report(filepath, mode, decode_enabled):
    ...
    for url, title in all_bookmarks:
        ...
        if key not in processed_items:
            ...
            # 当发现重复时，我们将原始的、未解码的项添加到移除列表，以便报告更清晰
...
def generate_report_file(original_filepath, duplicate_groups, mode, decode_enabled):
    ...
    with open(report_filename, 'w', encoding='utf-8') as f:
        ...
        f.write(f"去重模式: {mode}\n")
        f.write(f"URL 解码: {'已启用 (默认)' if decode_enabled else '已禁用'}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="【健壮版】清理HTML书签文件中的重复项，生成干净的新文件和详细报告。",
    ...
    parser.add_argument(
        ...
        action='store_false',
        dest='decode_enabled',
        help="【可选】禁用URL解码。默认情况下，脚本会自动解码URL。\n"
             "         使用此开关可按原始文本进行去重。"
    )
    
    args = parser.parse_args()
    deduplicate_and_generate_report(args.file, args.mode, args.decode_enabled)
