...
def parse_bookmarks_file(filepath, mode, filter_enabled, decode_enabled):
    ...
    for a_tag in all_a_tags:
        ...
        if url:
            normalized_url = url
            # 【核心改动】根据开关决定是否解码
            # BeautifulSoup 自动处理 & 等实体，这里只需处理 % 编码
            if decode_enabled:
...
def analyze_multiple_files(files, compare_mode, analysis_mode, filter_enabled, decode_enabled):
    ...
    report_lines.append("        多书签文件对比分析报告 (BeautifulSoup版)")
    report_lines.append("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="【BS4版】比较多个HTML书签文件，分析它们的交集或独有项。", formatter_class=argparse.RawTextHelpFormatter)
