...
def parse_bookmarks_file_fast(filepath, mode, filter_enabled, decode_enabled):
    ...
    total_folders_count = len(folders_set) # 注意：这里直接得到唯一文件夹数量

    print(f"解析完成: 找到 {total_bookmarks_count} 个书签 ({len(bookmarks_set)}个唯一), "
          f"{total_folders_count} 个唯一文件夹。")
    
    # 为了与您原有的函数返回格式保持一致
    # 注意: 流式解析器直接统计唯一文件夹，总数可以粗略认为等于唯一数
...
def main():
    """主函数，处理命令行参数。"""
    parser = argparse.ArgumentParser(description="【高级最终版】比较多个HTML书签文件，分析它们的交集或独有项。", formatter_class=argparse.RawTextHelpFormatter)
